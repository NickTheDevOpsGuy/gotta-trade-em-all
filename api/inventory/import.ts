...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { items } = (req.body ?? {}) as { items?: Array<{ card_id: string; quantity?: number }> };
  if (!Array.isArray(items) || items.length === 0 || items.length > 100) {
    return res.status(400).json({ error: 'items must be an array of 1–100 { card_id, quantity? } objects' });
  }
  ...
  for (const item of items) {
    ...
    if (!card_id || typeof quantity !== 'number' || quantity < 1 || quantity > 99) continue;
